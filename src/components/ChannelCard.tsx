...
export function ChannelCard({ slug, name, group, logo, href, featured, category }: Props) {
  ...
  return (
    <div
      className={`group card-hover relative flex cursor-pointer flex-col overflow-hidden rounded-2xl border bg-card-gradient shadow-card ${featured ? "border-primary/30" : "border-white/[0.06]"}`}
      title={`Play ${name}`}
      tabIndex={0}
    >
      {link}
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-black">
        <div
          ...
          style={{ background: `radial-gradient(ellipse 80% 70% at 50% 40%, ${meta.color} 0%, transparent 65%)` }}
        />
        ...
        <button
          ...
          className={`absolute right-2 top-2 z-[25] grid h-8 w-8 place-items-center rounded-full border border-white/10 bg-black/50 text-base backdrop-blur transition hover:bg-black/80 ${fav ? "text-yellow-300" : "text-white/60 hover:text-yellow-300"}`}
        >
          <span aria-hidden="true">{fav ? "★" : "☆"}</span>
        </button>
        <div className="pointer-events-none absolute bottom-2 right-2 z-[15] grid h-10 w-10 place-items-center rounded-full bg-primary text-primary-foreground shadow-glow transition duration-300 sm:translate-y-1 sm:opacity-0 sm:group-hover:translate-y-0 sm:group-hover:opacity-100">
      ...
      <div className="pointer-events-none relative z-[5] flex flex-col gap-1.5 border-t border-white/[0.05] bg-black/30 p-3">
        <div className="truncate text-[14px] font-bold leading-tight text-foreground">{name}</div>
        <div className="flex items-center justify-between gap-2">
          <span className={`inline-flex shrink-0 items-center rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.14em] ${meta.bg} ${meta.text}`}>{meta.label}</span>
