  ...
  return (
    <div className="min-h-screen bg-hero">
      ...
      <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
        ...
        <form
          onSubmit={submit}
          className="mt-8 space-y-4 rounded-2xl border border-white/10 bg-card/60 p-5 backdrop-blur"
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                className="w-full rounded-xl border border-white/10 bg-black/30 px-3 py-2 text-sm outline-none focus:border-primary"
              />
            </label>
            <label className="block">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-widest text-muted-foreground">
            ...
            {sources.map((s, i) => (
              <div key={i} className="grid grid-cols-[110px_1fr_auto] gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setSources((arr) =>
                      arr.map((x, j) => (i === j ? { ...x, quality: e.target.value } : x)),
                    )
                  }
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSources((arr) =>
                      arr.map((x, j) => (i === j ? { ...x, url: e.target.value } : x)),
                    )
                  }
        ...
        {channels.length === 0 ? (
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {channels.map((c) => (
              <li
                key={c.id}
                className="flex items-center justify-between gap-3 rounded-2xl border border-white/10 bg-card/60 p-4"
              >
                <div className="min-w-0">
                  <div className="font-semibold">{c.name}</div>
                  <div className="text-xs text-muted-foreground">
                    {CATEGORY_META[c.category].label} · {c.sources.length} source
                    {c.sources.length === 1 ? "" : "s"}
                  </div>
                </div>
                <button
                  onClick={() => remove(c.id)}
                  className="rounded-full border border-white/10 px-3 py-1.5 text-xs text-red-300 hover:bg-red-500/10"
                >
                  {t("settings.remove")}
                </button>
              </li>
