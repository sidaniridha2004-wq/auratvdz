...
function AdminPage() {
  ...
  const doBulk = async (active: boolean) => {
    ...
    try {
      await bulkFn({
        data: { password: getAdminPassword(), slugs: Array.from(selected), is_active: active },
      });
      setSelected(new Set());
      await invalidate();
    } catch (e) {
  ...
  const doInsert = async (patch: Patch & { slug?: string }) => {
    const slug = (patch.slug || patch.name || "")
      ...
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/(^-|-$)/g, "");
    if (!slug || !patch.name || !patch.stream_url)
      return alert("Name, stream URL, and slug are required.");
    setBusy(true);
    try {
      await insertFn({
        data: {
      ...
      <div className="w-full max-w-lg rounded-2xl border border-white/10 bg-card p-6 shadow-glow">
        <div className="flex items-start justify-between mb-4">
          <h2 className="font-display text-lg font-bold">
            {isNew ? "Add new channel" : `Edit · ${row.name}`}
          </h2>
          <button
            aria-label="Close"
            onClick={onClose}
            ...
            return (
              ...
              >
                ...
                <input
                  ...
                  onBlur={(e) =>
                    e.target.value !== r.title && doPatch(r.id, { title: e.target.value })
                  }
                ...
                <input
                  ...
                  onBlur={(e) =>
                    e.target.value !== r.subtitle && doPatch(r.id, { subtitle: e.target.value })
                  }
